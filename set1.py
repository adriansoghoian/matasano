...
def repeating_xor(ptxt, xor_keys):
    """
    Repeating key xor with a single character.
    """
    ctxt = ""

    for i, letter in enumerate(ptxt):
        key = xor_keys[i % len(xor_keys)]
        ctxt += chr(ord(letter) ^ ord(key)).encode('hex')
...
def problem_6(ctxt_path):
    ctxt = open(ctxt_path).read().decode('base64')
    print ctxt
...
if __name__ == "__main__":
    ...
    # ## Problem 5
    # ptxt = open("text/set1_problem5.txt").read()
    # keys = "ICE"
    ...
    # print desired_ctxt == repeating_xor(ptxt, keys)

    ## Problem 6
    # problem_6("text/set1_problem6.txt")
